...
export default function TutorRegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/login?registered=true&id=${encodeURIComponent(data.anonymousId)}&role=tutor`);
    } catch {
    ...
    <div className="card bg-base-100 shadow-sm border border-base-200 w-full">
      <div className="card-body gap-5 p-6 md:p-8">
        ...
        <div className="flex flex-col items-center gap-1 mb-2 text-center">
          ...
          <p className="text-xs text-base-content/60">Apply as a Peer Tutor mentor on Katuwang</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn btn-primary btn-sm w-full mt-2 text-white cursor-pointer text-xs"
          >
            {loading ? (
              <>
