...
import { ArrowLeft, Link as LinkIcon, Users, BadgeCheck, Plus, X, Lock, MapPin } from "lucide-react";
                ...
                >
                  <div className="flex flex-wrap gap-1.5 border border-base-300 rounded-lg bg-base-100 p-2.5">
                    {SUBJECT_TOPICS[subject].map((topic) => {
                      ...
                      const locked = usedTopics.includes(topic);
                      return (
                        <button
                          type="button"
                          key={topic}
                        ...
                        >
                          ...
                          {topic}
                          {!locked && (selected ? <X className="h-3 w-3" /> : <Plus className="h-3 w-3" />)}
                        </button>
                      );
                    })}
                  </div>
