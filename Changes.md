...
`docs/reviews/component-reuse-review-2026-09-10.md` **R3** (also
`design-review` D7 / `back-navigation` B6's "no keyboard way out of a dialog").

New `src/components/ui/Modal.tsx`: the backdrop + `modal-box` scaffold, closes on
**Escape** and backdrop click, **traps Tab focus** while open, and **restores
focus** to the trigger element on close. `bare` mode skips the built-in
header/close-button so a child can render its own chrome; otherwise it draws a
standard title row + `X`.

`ConfirmDialog` and `PromptDialog` now render their bodies through
`<Modal bare>` — pixel-identical (same `max-w-sm p-6 kt-card` box, same
`--kt-shadow-pop`), but every confirm/prompt dialog in the app gains Escape +
focus-trap + focus-restore for free. No API change to either.

The ~7 bespoke `modal modal-open` components (`AddSessionModal`,
`QuestionFormModal`, `ReportButton`, …) can migrate to `<Modal title>`
incrementally; not done here. 680/680, `tsc` clean.
...
`docs/reviews/repeated-markup-review-2026-09-10.md` **C1**. New
`src/components/ui/StatCard.tsx` — the `kt-stat` dashboard tile (label + big mono
value + tinted icon chip), rendering as a plain card or a hover-highlighted
`<Link>` when `href` is set. `tint` is now a token (`primary` / `secondary` /
`accent` / `success` / `error`) mapped to the `bg-{c}/10 text-{c}` pair inside
the component (all literal, so the Tailwind scanner keeps them).

- `admin/page.tsx`: `statCards` array `.map`s to `<StatCard>` (tint strings → tokens).
- `learner/page.tsx`: same, keeps `href`.
- `tutor/page.tsx`: the four **hand-unrolled** tiles (two `<div>` + two `<Link>`,
  ~42 lines) collapse to four `<StatCard>` lines.

Markup and classes are unchanged from what each dashboard rendered before.
`MyProgressView` / `SessionTestResults` use `kt-stat-value` in a different
layout and are out of scope. 680/680, `tsc` clean.
...
`docs/reviews/repeated-markup-review-2026-09-10.md` **C2**. New
`src/components/ui/BackLink.tsx` — the `‹ Back to X` ghost-button link
(`btn btn-ghost btn-sm text-xs gap-1.5` + `<ArrowLeft className="h-3.5 w-3.5" />`)
as one component taking `href` + children. Migrated 7 detail-page / sub-view
headers: `admin/users/[id]`, `tutor/students/[studentId]`,
`learner/tutors/[tutorId]` pages, and `ClassDetailsView`, `EditClassForm`,
`SessionTestResults`, `ProfileEditView` components. `ProfileEditView`'s
inline-text variant is normalised to the ghost-button look in the process.
No behaviour change; the contextual `router.back()` fallback (B4) is deliberately
left for the back-navigation pass. Unused `ArrowLeft` / `next/link` imports
removed from the touched files. 680/680, `tsc` clean.
